...
<script setup>
    ...
    const fetchDataPrices = () => {
        ProductsStore.GET_PRODUCTS_BY_PRICE(route.query.id, route.query.price, locale.value)
    };
    const fetchData = () => {
        ProductsStore.GET_SUB_PRODUCTS(name, route.query.id, 1, locale.value)
    };
    const onPageChange = (page) =>{
        ProductsStore.GET_SUB_PRODUCTS(name, route.query.id, page, locale.value)
    }
